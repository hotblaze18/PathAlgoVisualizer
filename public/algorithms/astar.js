...
export default class ASTAR {
    ...
    setHeap() {
        ...
        const comp = (nodeA, nodeB) => {
            return (nodeA.dist + nodeA.h) <= (nodeB.dist + nodeB.h);
        }
    ...
    setHeuristic() {
        const endR = this.board.endNode.row;
        const endC = this.board.endNode.col;
        
        for(let row=0; row<this.rows; row++) {
            for(let col=0; col<this.cols; col++) {
                if(this.nodes[row][col].status === "start")
                    continue;
                this.nodes[row][col].h = Math.sqrt(Math.pow(row-endR, 2)+ Math.pow(col-endC, 2)); 
            }
        }
    }
